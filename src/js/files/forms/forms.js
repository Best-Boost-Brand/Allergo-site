...
export function formQuantity() {
	document.addEventListener("click", function (e) {
		...
		if (targetElement.closest('[data-quantity-plus]') || targetElement.closest('[data-quantity-minus]')) {
			...
			// Перевіряємо, чи знайдено quantityWrapper
			if (quantityWrapper) {
				...
				// Оновлюємо значення в полі інпуту
				valueElement.value = value;
				
				// Перевіряємо, якщо значення дорівнює 0, то ховаємо блок
				if (value === 0) {
...
*/
// робочий код але спробуємо трохи інаклше
/*
export function formQuantity() {
	document.addEventListener("click", function (e) {
		let targetElement = e.target;
		...
		// Якщо натиснуто на .nav__img (label)
		if (targetElement.closest('.nav__img')) {
			...
			if (quantityWrapper) {
				...
				// Якщо значення дорівнює 0, встановлюємо 1
				if (value === 0) {
					value = 1;
					valueElement.value = value;
					quantityWrapper.style.display = ''; // Показуємо блок
				}
			}
		}
	});
...
function handleQuantityClick(e) {
  ...
  // Перевіряємо, чи натиснуто кнопку плюс або мінус
  if (
    ...
    // Перевіряємо, чи знайдено quantityWrapper
    if (quantityWrapper) {
      const valueElement = quantityWrapper.querySelector(
        "[data-quantity-value]"
      );
      let value = parseInt(valueElement.value, 10); // Конвертуємо в число
      ...
      // Перевіряємо, якщо значення дорівнює 0, то ховаємо блок quantity і показуємо кнопку
      if (value === 0) {
        const targetId = quantityWrapper.getAttribute("data-target");
        quantityWrapper.style.display = "none";
        ...
        const button = document.querySelector(
          `.nav__busket-btn[data-target="${targetId}"]`
        );
        if (button) {
  ...
  if (targetElement.closest(".nav__img")) {
    const navImgElement = targetElement.closest(".nav__img");
    const targetId =
      navImgElement.previousElementSibling.getAttribute("data-target");
    quantityWrapper = document.querySelector(
      `[data-quantity][data-target="${targetId}"]`
    );
    ...
    if (quantityWrapper) {
      ...
      if (value === 0) {
        value = 1;
        valueElement.value = value;
        quantityWrapper.style.display = "block"; // Показуємо блок quantity
      ...
      if (quantityWrapper) {
        ...
        if (value === 0) {
          ...
          const targetCheckbox = document.querySelector(
            `.basket-checkbox[data-target="${targetId}"]`
          );
          targetCheckbox.checked = false;
          quantityWrapper.style.display = "none";
        }
...
export function formQuantity() {
  document.addEventListener("click", function (e) {
    ...
    // Обробка натискань на кнопки плюс і мінус
    if (
      targetElement.closest("[data-quantity-plus]") ||
      targetElement.closest("[data-quantity-minus]")
    ) {
      ...
      if (quantityWrapper) {
        ...
        // Ховаємо блок quantity, якщо значення 0
        if (value === 0) {
          const targetId = quantityWrapper.getAttribute("data-target");
          const targetCheckbox = document.querySelector(
            `.basket-checkbox[data-target="${targetId}"]`
    ...
    if (targetElement.closest(".nav__img")) {
      ...
      if (quantityWrapper) {
        ...
        // Показуємо блок quantity і встановлюємо значення 1, якщо значення 0
        if (value === 0) {
          value = 1;
          valueElement.value = value;
          quantityWrapper.style.display = "block"; // Показуємо блок
        }
      }
    }
  });
...
export function formRating() {
  ...
  if (ratings) {
    ratings.forEach((rating) => {
      ...
      const ratingSize = +rating.dataset.ratingSize
        ? +rating.dataset.ratingSize
        : 5;
